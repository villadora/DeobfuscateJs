    ...
    nodetype.prototype.travel = function(before, after) {
        for(var l = this._travelable.length; l-- > 0;) {
            ...
            if((prop in this) && this[prop]) {
                var origin = this[prop];
                if(before) before.call(this, prop, origin);
                if(origin.forEach && origin.concat && typeof origin.length == 'number') {
                    var ary = this[prop];
                ...
                }else if(origin.travel instanceof Function){
                    origin.travel(before, after);
                }

                if(after) after.call(this, prop, origin);
            }
        }
    };
}
